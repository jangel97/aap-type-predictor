"""Benchmark Gemini on AAP: function calling vs text baseline vs zero-shot TCR.

Three strategies:
  1. function_calling — pass 1,060 tools, let model select directly
  2. text_baseline    — list 1,060 tools as text, ask for JSON array
  3. zero_shot_tcr    — list 79 entity types, ask model to predict source/target,
...
Usage:
    ...
    python benchmark_gemini.py --model gemini-2.5-pro
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark Gemini on AAP tool selection")
