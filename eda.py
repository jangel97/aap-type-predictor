"""Exploratory data analysis for the AAP type predictor dataset.
...
def print_stats(graph, train, test):
    ...
    for s, c in src_counts.most_common(10):
        print(f"  {s:<30} {c:>4} ({c / len(train) * 100:.1f}%)")

    print(f"\nTarget type distribution (top 10):")
    for t, c in tgt_counts.most_common(10):
        print(f"  {t:<30} {c:>4} ({c / len(train) * 100:.1f}%)")
...
def plot_training_pair_heatmap(train):
    ...
    ax.set_xticklabels(tgt_types, rotation=90, fontsize=6)
    ax.set_yticks(np.arange(len(src_types)) + 0.5)
    ax.set_yticklabels(src_types, fontsize=6)
...
def plot_test_pair_support(train, test):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.stripplot(x=categories, y=supports, order=order, hue=categories,
