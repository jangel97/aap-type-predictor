...
the 47 fixed held-out test queries. Reports mean +/- std across folds.
...
def main():
    ...
    torch.save({
        ...
        "cv_folds": args.folds,
        "cv_mean_f1": float(np.mean([r["f1"] for r in fold_results])),
        "cv_std_f1": float(np.std([r["f1"] for r in fold_results])),
    }, out_dir / "heads.pt")
